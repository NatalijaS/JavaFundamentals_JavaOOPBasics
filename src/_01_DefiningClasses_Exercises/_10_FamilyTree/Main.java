...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        while (!(line = reader.readLine()).equals("End")) {
            ...
            setInput(line, fullDataPeople, halfDataPeople, input);
        }
        connectInformation(fullDataPeople, halfDataPeople);
    ...
    private static void connectInformation(List<Person> fullDataPeople, List<Person> halfDataPeople) {
        halfDataPeople.forEach(partialPerson -> fullDataPeople.stream()
                .filter(fullPerson -> fullPerson.getBirthday().equals(partialPerson.getBirthday()) ||
    ...
    private static void setInput(String line, List<Person> fullDataPeople, List<Person> halfDataPeople, String[] input) {
        switch (input.length) {
            case 1:
